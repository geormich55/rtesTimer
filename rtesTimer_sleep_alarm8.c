...
int main(int argc,char **argv){
	...
	for (i=0;i<sample_number;i++){
		pthread_create(&tid[i],NULL,get_timestamp,(void*) i);//create pthread//
		if(i>1){
			error=(int)(timestamp[i]-timestamp[i-1])-period_usec;//compute error//
		}
		else{
			error=0;
		}
		usleep(period_usec+error);//sleep in time equal to period_usec+error//
	}
	for (i=i;i<sample_number;i++){
		pthread_join(tid[i],NULL);
	}
	FL=fopen("timestamps3.txt","w+");//write results to file//
	fwrite(&timestamp[0],sizeof(long),sample_number,FL);
	fclose(FL);
	printf("Saved to timestamps3.txt\n");
